            ...
            {RECENT_SCORES.map((entry, i) => {
              ...
              return (
                <div
                  key={entry.player}
                  style={style}
                  className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-3 border-b border-border-neon px-4.5 py-2.75 font-mono text-[13px] motion-safe:animate-tick"
                >
                  <span
        ...
        <div className="border border-border-neon bg-surface">
          ...
          <div className="flex flex-col gap-2.5 px-4.5 py-4.5">
            {TOP_PLAYERS_TODAY.map((row, i) => {
              ...
              const bgClass =
                PODIUM_BG_CLASS[podiumIndex] ??
                "bg-gradient-to-r from-cyan/6 to-transparent";
              const style = { width: `${100 - i * 16}%` } as CSSProperties;
              return (
                <div
                  key={row.player}
                  className="relative grid grid-cols-[36px_1fr_auto] items-center gap-2.5 py-2 font-mono"
                >
                  <div
                    style={style}
                    className={`pointer-events-none absolute inset-y-0 left-9 ${bgClass}`}
                  />
                  <span
                    className={`relative font-pixel text-[10px] ${rankClass}`}
                  >
                    #{pad2(row.rank)}
                  </span>
                  ...
                  <span
                    className={`relative font-pixel text-[11px] ${scoreClass}`}
                  >
